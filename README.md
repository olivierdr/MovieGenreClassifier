...
I chose two strategies to adress this problem: A traditional and interpretable method (TF-IDF + Logistic Regression) & a more modern and semantic approach (SentenceTransformer + Neural Network). The goal was to evaluate whether explicit word usage (captured by TF-IDF) or contextual meaning (captured by embeddings) would perform better on this multi-class text classification task, especially given the small dataset and class imbalance.
...
### SentenceTransformer Embeddings + Neural Network

The second approach uses embeddings generated by `SentenceTransformer('all-MiniLM-L6-v2')` to represent synopses as dense vectors. A custom deep neural network processes these embeddings, using batch normalization, dropout, and progressive dimensionality reduction. And I manually computed class weights (inverse of class frequency) to be applied in the loss function to handle imbalance.
